...
- Social share fallback dialog and share-event tracking

## Why this remake is simpler

The previous version mixed DaisyUI migration, graph libraries, and multiple UI patterns. This version removes DaisyUI entirely and avoids compatibility-heavy pieces. It uses local UI primitives and a small SVG graph so the app is easier to run, debug, and deploy.
...
If you previously saw an error like `ENOTFOUND packages.applied-caas-gateway1.internal.api.openai.org`, delete the old install artifacts first:
...
That error is caused by a lockfile generated inside an internal sandbox registry, not by the React or Supabase code.


## Image compression notes

The CMS image upload does not add a heavy dependency. It uses browser APIs:

- `HTMLCanvasElement` resizes images to a maximum of 1600×1200.
- Output defaults to WebP with JPEG fallback.
- The compressed file is uploaded to Supabase Storage bucket `portfolio-media`.
- The public image URL is automatically copied into the current blog/project image field.

For production, keep the bucket public only for portfolio images. Do not use this bucket for private files.
